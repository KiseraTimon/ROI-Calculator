document.getElementById('roiForm').addEventListener('submit', function (e) {
    ...
    */
    ...
    // Formatting Values
    const currency = amount => `$${amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
    const percentage = value => `${value.toFixed(2)}%`;

    // Rendering Output
    resultDiv.innerHTML = `
        <div>
          <div><span class="label">✅ Total Energy Savings:</span> <span class="value">${currency(totalSavings)}</span></div>
          <div><span class="label">✅ Net Profit:</span> <span class="value">${currency(netSavings - investment)}</span></div>
          <div><span class="label">✅ ROI:</span> <span class="value">${percentage(roi)}</span></div>
        </div>
      `;
});